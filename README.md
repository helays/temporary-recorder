...
窗口是**无边框**的，顶部只有一行：图标 + 菜单 + 窗口按钮（与 VS Code 同形态），
自上而下是 **标题行 → 标签栏 → 编辑器 → 状态栏**。
...
菜单里有什么：**文件**（新建 / 打开 / 保存 / 另存为 / 关闭 / 退出）、
**设置**（设置… / 打开临时目录 / 清理临时文件）、
**编辑**（撤销重做 / 剪切复制粘贴 / 全选 / 查找 / 替换 / 转到行）、
**查看**（格式化 / 压缩 / 主题）、**帮助**（使用说明 / 关于 / 打开数据目录）。
每个菜单项右侧会显示对应按键，完整键位见下一节。

界面上的其它操作：

- **标签**：双击标签名重命名（只改显示名，不改文件名）；拖动标签排序
- **窗口**：拖动标题栏空白处移动；拖动四条边缩放；双击标题栏空白处最大化 / 还原
- **菜单**：`←` `→` 换顶级菜单，`↑` `↓` 移动高亮，`Enter` 执行，`Esc` 关闭
